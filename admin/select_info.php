...
if($select_run){
    if($select_run && mysqli_num_rows($select_run) > 0){
        while($row = mysqli_fetch_array($select_run)){
            ...
            <div>
            <a href='' target="_top"><i class="fas fa-eye" ></i> </a> &nbsp; <a onclick="myFunction()"> <i class="fas fa-folder"></i></a>  &nbsp; </a> <?php if($student_confirm != NULL){ echo "<a href='select_status.php?id=$inc_student'><i class='far fa-lightbulb'></i> </a>";}else{echo "<a href='approvals.php?id=$studentinc'><i class='fas fa-edit'></i> </a>"; } ?> 
            </div>
