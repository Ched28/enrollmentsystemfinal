            ...
            if($run_course){
                if($run_course && mysqli_num_rows($run_course) > 0 ){
                    while($row = mysqli_fetch_array($run_course))
                    {
                        ...
                        echo "<div> <div><h3> $sectionname <br><span style='font-size: .8em;'> Number Of Students: &nbsp; $studentcount </span></h3></div> <div> <ul><li><a href='masterlist.php?sec=$sectionname'><i class='fas fa-list'></i> &nbsp; Master List</a></li>  <li><a href='schedules/section_subjlist.php?sec=$sectionname'><i class='fas fa-calendar-alt'></i> &nbsp; Schedule </a></li></ul></div></div>";
